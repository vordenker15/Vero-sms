import subprocess, sys, os, time, pygetwindow
try:
...
def send_service(number, service):
    ...
    if result[0] == True:
        all_sends += 1
        success_sends += 1
        print(f"[+] {success_sends} {result[1]}")
    else:
        all_sends += 1
        failed_sends += 1
        print(f"[-] {failed_sends} {result[1]}")
        os.system(f"title Giden:{success_sends} Gitmeyen:{failed_sends} Toplam:{all_sends} Numara:{number}")
...
def watermark():
    win = pygetwindow.getWindowsWithTitle('SHV44_SMS_Bomeber')[0]
    win.size = (1369, 679)
    Write.Print(f"""
    \t\t                               .*/#%%%%%%%%%%%%%%%%%%%%%(/*.                               .,,**********************,,.
    \t\t                         ./#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#/.                   .,************************************,.
...
def get_number():
    ...
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın. Şunun gibi: "5xxxxxxxxx" (Sadece Türkiye numaralarında çalışır!)\n[?] : """))
